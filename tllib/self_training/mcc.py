...
class ImageClassifier(ClassifierBase):
    def __init__(self, backbone: nn.Module, num_classes: int, bottleneck_dim: Optional[int] = 256, **kwargs):
        print(f"Backbone output shape: {backbone.out_features}")
        bottleneck = nn.Sequential(
            # nn.AdaptiveAvgPool2d(output_size=(1, 1)),  # 保证输出的特征图是1x1
            # nn.Flatten(),  # 将多维张量展平为二维
...
#
#         #print(f"Backbone output shape: {backbone.out_features}")  # 验证输出特征维度
#         self.backbone = backbone  # 保存 backbone
#
#         # 定义 bottleneck 层
#         self.bottleneck = nn.Sequential(
#             nn.Linear(768, bottleneck_dim),  # 输入维度改为 768，输出维度为 bottleneck_dim
#             nn.BatchNorm1d(bottleneck_dim),  # 批量归一化
#             nn.ReLU()  # ReLU 激活函数
#         )
#
#         # 定义分类层
#         self.classifier = nn.Linear(bottleneck_dim, num_classes)  # 分类层
#
#     def forward(self, x):
#         # 获取 backbone 的输出
#         features = self.backbone(x)  # 特征提取
#         #print(f"Output shape from backbone: {features.shape}")  # 打印输出特征形状
#
#         # 使用 [CLS] token 的输出（features[:, 0, :]）
#         cls_output = features[:, 0, :]  # 取第一个 token，维度为 (batch_size, 768)
#
#         # 应用 bottleneck 层
#         bottleneck_output = self.bottleneck(cls_output)  # 通过 bottleneck
#         # 进行分类
#         logits = self.classifier(bottleneck_output)  # 获取分类结果
#
#         return logits, bottleneck_output  # 返回 logits 和 bottleneck 输出
